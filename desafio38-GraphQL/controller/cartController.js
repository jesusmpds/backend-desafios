...
exports.getCartByUserId = async (req, res, next) => {
  try {
    const { _id } = req.user;
    const shoppingCart = await cart.getAllCartItems(_id);
    return shoppingCart;
  } catch (error) {
    logger.error(error);
  }
...
exports.addShoppingCartByUserId = async (req, res, next) => {
  try {
    ...
    const shoppingCartCreated = await cart.addCart({
      user: _id,
      products: product,
    });
...
exports.updateorCreateShoppingCartByUserId = async (req, res, next) => {
  ...
  const shoppingCartUpdated = await cart.updateCart(_id, products);
  res.send(shoppingCartUpdated);
};

exports.deleteShoppingCartByUserId = async (req, res, next) => {
  const { _id } = req.user;
  const deleted = await cart.deleteCart(_id);
  res.send(deleted);
};
