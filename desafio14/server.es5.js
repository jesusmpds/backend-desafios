...
var APP = express(); //socket.io server
...
var io = new Server(server); //Handlebar Engine
...
io.on('connection', function (socket) {
  ...
  socket.on('newMessage', function (mensaje) {
    ...
    console.log(mensaje);
    console.log(messages); //Emitir nuevo mensaje al cliente
...
productosAPIRouter.get('/productos/:id', function (req, res) {
  var productoByID = productos.leer().find(function (producto) {
...
productosAPIRouter.put('/productos/:id', function (req, res) {
  var foundIndex = productos.leer().findIndex(function (producto) {
    return producto.id === parseInt(req.params.id);
  });
...
productosAPIRouter["delete"]('/productos/:id', function (req, res) {
  productos.productos = productos.leer().filter(function (producto) {
    return producto.id !== parseInt(req.params.id);
  });
...
server.listen(PORT, function () {
  return console.log("Servidor en el puerto ".concat(PORT));
});
