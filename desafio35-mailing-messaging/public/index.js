...
//Chat
    ...
    if (inputEmail.value && messageInput.value && inputFirstName.value) {
        ...
        messageInput.value = '';
            ...
            alert('please add you Name, Email , and add a message')
        }
    });
    ...
    socket.on('allMessages', (msgs)=> {
        msgs.forEach( msg => {
            ...
            item.innerHTML = `<span class="badge bg-dark">${msg.author.name}</span> <span>${msg.date}</span>: <span>${msg.text}</span>`;
    ...
    // Event listener for sending SMS if word 'administrador' is used within chat messages
